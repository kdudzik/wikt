...
\end{enumerate}
Wybór padł na rozwiązanie drugie z~kilku powodów. Okazało się, że dostęp do standardowych elementów w~trakcie działania nowego formularza jest bardzo przydatny, przede wszystkim zaś ich ukrywanie (za pomocą metody jQuery \kod|.hide()|, zmieniającej atrybut CSS \kod|display| na \kod|none|) umożliwia proste przełączanie między starym a~nowym formularzem w~trakcie edycji, co było jednym z~wymagań funkcjonalnych aplikacji.

Należało zatem zapewnić prawidłową synchronizację pomiędzy polem \kod|wpTextbox1| a~polami nowego formularza. Najważniejsze było podpięcie odpowiednich zdarzeń pod przyciski wysyłające formularz na serwer. Funkcje wówczas wywoływane opisano w~podsekcji~\ref{impl:printer}. Z~kolei po zmianach w~starym formularzu konieczne jest ponowne parsowanie wikikodu (p.~podsekcja~\ref{impl:parser}).
...
%\begin{illustration}
%	\fbox{\includegraphics{newform2}}
%	\caption{Fragment nowego formularza w~haśle z~wieloma sekcjami językowymi}
%	\label{fig:newform2}
%\end{illustration}
...
\subsection{Drukowanie wyników działania aplikacji}
\label{impl:printer}
...
