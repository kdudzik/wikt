\label{chap:impl}
W~ostatnim rozdziale został szczegółowo przedstawiony proces implementacji aplikacji. Opisano użyte technologie i~metodologię, a~szczególną uwagę poświęcono problemom, które powstały podczas tworzenia programu. Pierwszy podrozdział zawiera opis środowiska, w~którym osadzona jest aplikacja, natomiast kolejne dwa skupiają się na dwóch logicznie wyodrębionych częściach projektu: uproszczonym formularzu edycyjnym oraz automatyzacji tworzenia haseł. W~ostatniej części rozdziału przedstawiono przebieg wdrożenia aplikacji w~polskim Wikisłowniku i~możliwości dalszego rozwoju.
...
\begin{enumerate}
	...
	\item tzw. gadżety, czyli dodatkowe skrypty rozszerzające funkcjonalność, możliwe do włączenia w~preferencjach użytkownika --- także one są edytowane i~wybierane przez administratorów,
...
\subsubsection{Git}
Systemem kontroli wersji używanym w~projekcie był Git. Dzięki niemu możliwe było uniknięcie problemów związanych z~zarządzaniem wersjami. Dla bezpieczeństwa kod był na bieżąco wysyłany do zdalnego repozytorium, założonego w~serwisie GitHub~\cite{github}. Kontroli wersji podlegała również niniejsza praca i~jej kod źródłowy w~systemie \XeTeX. Historia zmian aplikacji i~pracy jest dostępna do wglądu na serwerach GitHub~\cite{github:wikt}.
...
Ciekawym problemem było ułożenie pól formularza w~taki sposób, by zajmował on możliwie mało miejsca, a przy tym pozostał przejrzysty. Każda sekcja językowa składa się z~kilkunastu podsekcji, dlatego należało dobrze rozmieścić kilkanaście pól tekstowych, przy czym te mogły zawierać bardzo dużo tekstu (np. podsekcja \emph{tłumaczenia} w~popularnych polskich hasłach) albo pozostawać puste. Najprostszym, standardowym rozwiązaniem byłoby ustawienie stałej wysokości (np. równej 3~wierszom) dla każdego pola, które byłoby wyposażone w~zwykły pasek przewijania. Takie pola w~większości przypadków zajmowałyby jednak zbyt wiele miejsca, przy tym formularz z~dużymi białymi plamami jest niezbyt estetyczny. Zdecydowano się na rozwiązanie spotykane czasem w~portalach internetowych, najlepiej znane z~Facebooka --- pola tekstowe zmieniają rozmiar w~trakcie wpisywania kolejnych znaków. Istnieje kilka wtyczek do jQuery o~podobnej funkcjonalności, jednak na potrzeby aplikacji konieczne było stworzenie takiej, która łączy cechy kilku z~nich. Problemem okazała się też wydajność, ponieważ niektóre hasła mają w~trybie edycji nawet ponad sto pól dynamicznie dostosowujących swój rozmiar. Rozwiązaniem jest dołączenie do strony ukrytego elementu \kod|<div>|, do którego na bieżąco kopiowana jest zawartość ukrytego pola tekstowego --- na podstawie jego wysokości ustawiana jest wysokość pola. Kod, który za to odpowiada, znajduje się w~pliku \kod|jquery.autoresize.js|.
...
Moduł \kod|EPrinter| odpowiada także za generowanie kodu z~danych pobranych w~sposób automatyczny. Niektóre z~tych funkcji zostały opisane w~kolejnym podrozdziale.

\section{Automatyzacja edycji hasła}
\label{sec:impl-auto}
Drugą obok uproszczenia ogólnego procesu edycji funkcją, jaką ma spełniać aplikacja, jest zautomatyzowanie niektórych działań wykonywanych podczas edytowania przez redaktorów. W~punkcie~\ref{wikt:drawbacks} wspomniano o~dużym poziomie redundancji, będącym prostą konsekwencją rozbicia projektu na wersje językowe i~niemożności zachowania pierwszej postaci normalnej danych w~bazie Wikisłownika. Spora część danych wprowadzanych do haseł jest już obecna gdzie indziej --- czy to w~innych hasłach polskiego Wikisłownika, czy też w~innych wersjach językowych. Wielu redaktorów wykorzystuje przykładowo inne edycje projektu do wprowadzania takich danych, jak wymowa w~międzynarodowym alfabecie fonetycznym (IPA), grafiki czy nagrania dźwiękowe, zaś na podstawie samej polskiej wersji możliwe jest chociażby kopiowanie przykładów użycia z~jednego hasła do drugiego.

\subsection{API MediaWiki}
Automatyzacja edytowania Wikisłownika (jak i~innych projektów Fundacji) byłaby niezwykle trudna bez interfejsu programistycznego dołączonego do każdej witryny opartej na MediaWiki~\cite{mw:api}. API umożliwia uproszczony dostęp do bazy Wikisłownika w~licznych formatach wymiany danych, jak XML, JSON, YAML czy WDDX. Aby otrzymać odpowiedź na dowolne zapytanie, wystarczy pobrać dane z~adresu \url{http://pl.wiktionary.org/w/api.php} uzupełnionego o~odpowiednie parametry.
...
Dodatkową komplikacją jest występowanie IPA w~dwóch wariantach. Zapis w~nawiasach kwadratowych to zapis fonetyczny, natomiast między ukośnikami --- fonologiczny. W~różnych wersjach językowych używane są różne warianty, natomiast w~polskiej edycji podawane są oba. Aplikacja bierze pod uwagę, jak zbudowany jest szablon w~danym Wikisłowniku, i~w~zależności od tego pozwala na wstawienie IPA w~szablonie \kod|{{IPA}}| lub \kod|{{IPA3}}|.
...
\begin{illustration}
	\fbox{\includegraphics[width=0.65\textwidth]{iparesult}}
	\caption{Fragment okna wyświetlającego pobrane zapisy IPA}
	\label{fig:iparesult}
\end{illustration}

\subsubsection{Pobieranie grafik}
Innym elementem kopiowanym pomiędzy wersjami językowymi są ilustracje do haseł. Jest oczywiste, że ilustracja danego wyrazu będzie równie dobra w~każdej edycji Wikisłownika. Ponieważ znakomita większość używanych w~projektach Wikimedia grafik znajduje się na serwerach Wikimedia Commons (centralnego repozytorium plików dostępnych na wolnej licencji), a~w~polskich projektach używane są tylko takie grafiki, możliwe jest automatyczne odnajdywanie tych ilustracji.
...
\begin{illustration}
	\fbox{\includegraphics[width=0.65\textwidth]{picture}}
...
Lokalizacja miniatury ilustracji o~zadanej wielkości nie jest bezpośrednio zależna od nazwy pliku --- MediaWiki używa funkcji haszujących, aby umieścić ją w~jednym z~wielu katalogów. Dlatego przy użyciu zwykłych metod niezależnych od MediaWiki nie jest możliwe natychmiastowe wyświetlenie ilustracji bez zapytania do API Wikimedia Commons, które przekaże niezbędne dane. Po otrzymaniu tej informacji aplikacja ładuje odpowiednie obrazy i~wyświetla je po najechaniu myszą na nazwę pliku, jak pokazano na ilustracji~\ref{fig:pic}.
...
\begin{illustration}
	\fbox{\includegraphics[width=\textwidth]{example}}
...
Dzięki modułowej budowie dalsze udoskonalanie aplikacji nie nastręcza trudności. Formularz edycyjny przybrał formę, którą można uznać za stabilną --- dlatego kolejne funkcje prawdopodobnie będą rozszerzać zakres automatyzacji aplikacji. Dzięki łatwości korzystania z~API MediaWiki możliwości są pod tym względem naprawdę duże. Oto informacje, które mogłyby również być pobierane w~sposób automatyczny z~innych projektów lub samego polskiego Wikisłownika:
\begin{itemize}
...
Wikisłownik, jak każdy tego typu projekt, ma płynne zasady, które zmieniają się co pewien czas. Powstają też nowe rozwiązania techniczne usprawniające pracę. Przykładowo w~końcowej fazie implementacji aplikacji zaczęto w~Wikisłowniku stosować na szerszą skalę szablony odmiany --- koniugacja i~deklinacja są teraz podawane w~zupełnie inny sposób niż do tej pory. Warto zadbać, aby kolejna wersja formularza edycyjnego wspomagała używanie tych szablonów. Praca przy aplikacji nie została bynajmniej zakończona --- będzie kontynuowana w~sposób ciągły. Udało się jednak zamknąć kluczowy etap jej rozwoju i~stworzyć, miejmy nadzieję, produkt wysokiej jakości.
